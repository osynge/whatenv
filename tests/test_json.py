...
from osweint.nvclient_view_json2 import view_json_client
...
class testJson(unittest.TestCase):
    ...
    def test_current_session(self):
        ...
        json_view = view_json_client(self.mclient)
        session = self.connection.get_session_current()
        session_json = json_view.dump_session(session)
        log.error("session_json = %s" % (session_json))
